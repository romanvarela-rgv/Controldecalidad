...
public class EnemyFlyThrough : MonoBehaviour
{
    ...
    public float zigzagFrequency = 2f;    // Velocidad de oscilaci�n
    public float zigzagAmplitude = 0.5f;  // Amplitud del zigzag
    public GameObject bulletPrefab;

    private Vector2 moveDirection = Vector2.down; // Direcci�n inicial (por defecto hacia abajo)
    private float startAxisPosition; // Posici�n inicial para el zigzag
    ...
    private void Start()
    {
        ...
        // Autodestrucci�n despu�s de 8 segundos
        Invoke("SelfDestruct", 8f);
    }

    private void Update()
    {
        // Movimiento en la direcci�n definida con un zigzag en el eje perpendicular
    ...
    private void Shoot()
    {
        if (bulletPrefab != null)
        {
            ...
            bullet.GetComponent<Rigidbody2D>().velocity = moveDirection * 10f;  // Bala sigue direcci�n de movimiento
    ...
    {
        if (this != null)  // Verifica que el objeto a�n exista antes de destruirlo
        {
            Destroy(gameObject);
        }
    }
    ...
    private IEnumerator DamageCoroutine(float damage)
    {
        ...
        else
        {
            ...
            // Crear la animaci�n de muerte SOLO UNA VEZ
